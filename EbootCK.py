...
class MainWindow(QWidget, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        if os.name == 'posix':
            ...
            shift=[-181, 0]
        self.move(shift[0] + int(resolution.width() / 2) - int(self.frameSize().width() / 2), shift[1] + int(resolution.height() / 2) - int(self.frameSize().height() / 2))
    ...
    def Archive_Eboot(self):
        ...
        if out_folder and title:
            ...
            output_folder = os.path.join(out_folder,title)
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
            files = {'icon0':os.path.join('tools','tmp','icon0.png'), 
                     'icon1':os.path.join('tools','tmp','icon1.pmf'),
                     'pic0' :os.path.join('tools','tmp','pic0.png'), 
                     'pic1' :os.path.join('tools','tmp','pic1.png'), 
                     'snd0' :os.path.join('tools','tmp','snd0.at3'), 
                     'boot' :os.path.join('tools','tmp','boot.png')}
            for file in files:
                filename, file_extension = os.path.splitext(files[file])
                try:
                    shutil.copy(os.path.join(func.config_folder,files[file]), os.path.join(output_folder, file+file_extension))
                except Exception as e:
                    pass
                    #print(e)
            # Write metadata to readme
            with open(os.path.join(output_folder,'README.txt'), 'w') as readme:
            ...
            func.Take_Screenshot(os.path.join('tools','preview.html'), os.path.join(output_folder,'screenshot.png'))
        ...
        def OpenFileDialog():
            ...
            if fileName:
                ...
                if buttonID == "Icon1" and self.FFEditorEdit.text():
                    if os.path.isfile(self.FFEditorEdit.text()):
                        # Convert pmf files to .webm for preview
                        func.PVidConvert(fileName)
                    else:
                        self.make_ErrorMessage("Misconfigured FFMpeg path, skipping import conversion.")
                elif buttonID == "Snd0" and self.At3EditorEdit.text():
                    if os.path.isfile(self.FFEditorEdit.text()):
                        # Convert at3 files to .ogg for preview
                        func.PAudConvert(fileName)
                    else:
                        self.make_ErrorMessage("Misconfigured at3tool path, skipping import conversion.")
                self.setEnabled(True)
...
class PreviewWindow(QWidget, Ui_PreviewWindow):
    def __init__(self, parent=None):
        ...
        resolution = QtWidgets.QDesktopWidget().screenGeometry()
        self.move(int(240 + (resolution.width() / 2) - (self.frameSize().width() / 2)),int((resolution.height() / 2) - (self.frameSize().height() / 2)))
        self.setWindowFlags(QtCore.Qt.WindowTitleHint | QtCore.Qt.CustomizeWindowHint)
        self.webView.load(QtCore.QUrl("file:///"+os.path.join(func.config_folder,'tools','preview.html').replace("\\", "/")))
        self.webView.show()
        self.show()
    ...
    def update_preview(self, icon0, icon1, pic0, pic1, snd0, boot):
        ...
        tmp_folder = os.path.join(func.config_folder,"tools","tmp")
        ...
        for file in files:
            filename, file_extension = os.path.splitext(files[file])
            try:
                shutil.copy(files[file], os.path.join(tmp_folder,file+file_extension))
            except Exception as e:
                pass
                #print(e)
        ## Reload webview
...
class Functions():
    def __init__(self):
        ...
        self.tmp_folder = os.path.join(self.config_folder,"tools","tmp")
    ...
    def SaveOpts(self, ff_opts='-an -c:v rawvideo -y -vf scale=144:80,setsar=1/1,setdar=9/5', audedit='', imgedit='', videdit='', at3tool='', ffmpeg='', umdedit='', output='', author='', contact='', comment=''):
        ...
        with open(os.path.join(self.config_folder,'EbootCK.ini'), 'w') as configfile:
            config.write(configfile)
    def LoadOpts(self):
        config = configparser.ConfigParser()
        configfile = os.path.join(self.config_folder,'EbootCK.ini')
    ...
    def Take_Screenshot(self, url, outfile):
        output = outfile
        webViewport = QtWebEngineWidgets.QWebView()
    ...
    def At3Convert(self, InFile, OutFile):
        TmpFile = os.path.join(self.tmp_folder,'audio.wav')
    ...
    def PVidConvert(self, InFile):
        OutFile = os.path.join(self.tmp_folder, 'video.webm')
        command = '"{0}" -i "{1}" -y "{2}"'.format(self.config['Settings']['ffmpeg'], str(InFile), str(OutFile))
        print(command)
        ffconvert = subprocess.call(command, shell=True)
    def PAudConvert(self, InFile):
        OutFile = os.path.join(self.tmp_folder, 'audio.ogg')
        TmpFile = os.path.join(self.tmp_folder, 'snd0.wav')
        # Convert At3 to wav
    ...
    def get_mps_info(self, InFile):
        ...
        ffprobe = os.path.join(os.path.dirname(self.config['Settings']['ffmpeg']),ffprobe_exec)
